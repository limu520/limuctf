    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                this.language.modalClose = this.language.modalClose || '取消';//AXUI
                this.language.edit = this.language.edit || {};
                this.language.edit = { title: this.language.edit.title || '修改数据',//AXUI
                                       button: this.language.edit.button || '修改'//AXUI
                                     };
                this.language.delete = this.language.delete || {};
                this.language.delete = { title: this.language.delete.title || '删除数据',//AXUI
                                         button: this.language.delete.button || '删除' };//AXUI
                this.language.add = this.language.add || {};
                this.language.add = { title: this.language.add.title || '添加数据',//AXUI
                                      button: this.language.add.button || '添加'//AXUI
                                    };
                this.language.success = this.language.success || '操作成功!';//AXUI
                this.language.error = this.language.error || {};
                this.language.error = { message: this.language.error.message || '发生未知错误!',//AXUI
                                        label: this.language.error.label || '出错了!',//AXUI
                                        responseCode: this.language.error.responseCode || '响应代码: ',//AXUI
                                        required: this.language.error.required || '必填项',//AXUI
                                        unique: this.language.error.unique || '有重复字段'//AXUI
                                      };

                var modal =
                '<div class="ax-window" data-mask="true" data-drag="true" id="' + modal_id + '">' +
                ...
                '<div class="ax-window-title"></div>' +
                '<div class="ax-break-line"></div>' +
                '<div class="ax-window-content ax-emulate">' +
                '</div>' +
            ...
            _openDeleteModal: function () {
                ...
                for (var j in columnDefs) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        data += "<div class='ax-form-group'><div class='ax-flex-row'><div class='ax-form-label' for='"
                            + that._quoteattr(columnDefs[j].name)
                            + "'>"
                            + columnDefs[j].title
                            + "：</div><div class='ax-form-input'><input  type='hidden'  id='"
                            ...
                            + fvalue
                            + "</input></div></div></div></div><div class='ax-break-line'></div>";
                    }
                }//AXUI

                var selector = this.modal_selector;
                //var fill = function () {//AXUI
                    var btns = '<a href="###" class="ax-btn ax-text ax-ignore ax-window-close">' + that.language.modalClose + '</a>' +
                        '<button type="submit" class="ax-btn ax-primary" id="deleteRowBtn">' + that.language.delete.button + '</button>';
                    $(selector).find('.ax-window-title').html(that.language.delete.title);//AXUI
                    $(selector).find('.ax-window-content').html(data);//AXUI
                    $(selector).find('.ax-btns').html(btns);//AXUI
                    var modalContent = $(selector).find('.ax-window-contain');//AXUI
                ...
                //};//AXUI
                $(selector).addClass("ax-window-show");//AXUI
                $(selector+"[data-drag='true'] .ax-window-title").mousedown(function(e)
                {
                ...
                //this.internalOpenDialog(selector, fill);//AXUI
                $(selector + ' input[0]').trigger('focus');
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                for (var j in columnDefs) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        if(columnDefs[j].inline){ //to add upto 4 inline columns
                            if(count==0) {
                                count++;
                                data += "<div class='ax-form-group' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) + "'><div class='ax-flex-row'>";
                                data += "<div for='" + columnDefs[j].name + "'  class='ax-form-label'>" + columnDefs[j].title + ":</div></div>";
                            ...
                            data += "<div class='ax-form-group' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) +"'><div class='ax-flex-row'>";
                            data += "<div for='" + columnDefs[j].name + "' class='ax-form-label'>" + columnDefs[j].title + ":</div>";
                            data += "<div class='ax-form-input'>";
                        }
                        ...
                        data += "</div>";
                        data += "<div id='" + this._quoteattr(columnDefs[j].name) + "label"
                                + "' class='ax-valid'></div>";
                        if(!columnDefs[j].inline || (+j+1 < columnDefs.length && !columnDefs[+j+1].inline)) {
                            data += "</div></div><div class='ax-break-line'></div>";//AXUI
                        }
                        else
                            data += "</div></div><div class='ax-break-line'></div>";
                    }
                }//AXUI
                // data += "</form>";
                ...
                var selector = this.modal_selector;
                //var fill = function () { //AXUI
                var btns = '<a href="###" class="ax-btn ax-text ax-ignore ax-window-close">'+closeCaption+'</a>' +
                    '<button type="submit" class="ax-btn ax-primary" id="'+buttonClass+'">'+buttonCaption+'</button>';
                    $(selector).find('.ax-window-title').html(title);//AXUI
                    $(selector).find('.ax-window-content').html(data);//AXUI
                    $(selector).find('.ax-btns').html(btns);//AXUI
                    var modalContent = $(selector).find('.ax-window-contain');//AXUI
                    if (modalContent.parent().is('form')) {
            ...
            _deleteRowCallback: function (response, status, more) {
                    var selector = this.modal_selector;
                    $(selector + ' .ax-window-content .alert').remove();//AXUI
                    ...
                    if (this.closeModalOnSuccess) {
                        ...
                        var message = '<div class="alert alert-success" role="alert">' +
                            ...
                            '</div>';
                        $(selector + ' .ax-window-content').append(message);//AXUI
                    }
